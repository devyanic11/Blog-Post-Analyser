...
def show_info():
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.write(
        "Thanking you for reading this far😇. I am working on this app to add new features and make it more interesting. Feel free to reachout to suggest ideas or give a feedback.")
...
def display(result):
    ...
    with col1:
        col1.metric("Positive Score", str(result["positive_score_num"]) + "%",
                    help="Indicates how positive sentence the is")
        # st.markdown("<span style='font-size: 1.2rem;'>Positive Score</span>", unsafe_allow_html=True)
        # st.header("83")
    with col2:
        col2.metric("Negative Score", str(result["negative_score_num"]) + "%",
                    help="Indicates how negative sentence the is")
        # st.markdown("<span style='font-size: 1.2rem;'>Negative Score</span>", unsafe_allow_html=True)
        # st.header("65")
    with col3:
        col3.metric("Polarity Score", str(result["polarity_score"]),
                    help="Polarity scores are numerical values that range from -1 to 1, where -1 indicates a very negative sentiment, 0 indicates a neutral sentiment, and 1 indicates a very positive sentiment.")
        # st.markdown("<span style='font-size: 1.2rem; text-align: center;'>Polarity Score</span>", unsafe_allow_html=True)
        # st.header("93")
    with col4:
        col4.metric("Subjective Score", str(result["subjective_score"]),
                    help="Subjectivity scores are numerical values that range from 0 to 1, where 0 indicates a very objective text, and 1 indicates a very subjective text. ")
    ...
    with col5:
        ...
        st.write(
            "The Gunning Fog formula generates a grade level between 0 and 20. It estimates the education level required to understand the text. According to your fox index score given text is likely to be understood by:")
